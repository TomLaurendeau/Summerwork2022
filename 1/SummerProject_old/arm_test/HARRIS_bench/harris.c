...
struct pointData * getMaximaPoints(float *responsedata, int width, int height, float percentage, int filterRange, int suppressionRadius, int lowerBound, int upperBound)
{
	...
	free(globalHarriscorners);
	free(maximaSuppressionMat);
	free(intr_points);
	return top_points;
}
...
void forkHarris(int currentCore)
{
	int size = Width*Height;
	int filter = 3;
	unsigned char *globalData = (unsigned char*)malloc(sizeof(unsigned char)*size);
	float * globalResponse = (float*)malloc(sizeof(float)*size);
