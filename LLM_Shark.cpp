...
int main(int ac, char **av) {
	...
	if (LLM_DEPS == 1)
	{
		...
		//	fflush(stdout);
			...
			{
				printf("%s correlation %f \n", task_vector[j][0].event_name[t], task_vector[j][0].total_correlations[t-1]);

				if (strcmp(task_vector[j][0].event_name[t],"PAPI_L2_DCM") == 0)
				{
					cache_miss_index=t-1;
					if (task_vector[j][0].total_correlations[t-1] < -0.2) //-1 to take into the consideration that instr_ret is not included
					{
		...
		printf("Press any key to assign cache partitions according to correlation\n");
		getline();
		for (int j = 0; j < task_vector.size(); j++)
		{

			if (task_vector[j][0].total_correlations[cache_miss_index] < -0.2) // ---- LLM_shark test
			{
				//double normalization = fabs(task_vector[j][0].total_correlations[cache_miss_index])/corr_gen);
				double temp = (task_vector[j][0].total_correlations[cache_miss_index]/corr_gen)*PROFILING_PART_SIZE;
				task_vector[j][0].partition.size = round  (temp);;
				printf("Task %s cache partition size: %d\n", task_vector[j][0].name, task_vector[j][0].partition.size);
			}
			else
			{
				task_vector[j][0].partition.size = 1;
				printf("Task %s junk cache container\n", task_vector[j][0].name);
			}
			task_vector[j][0].partition.cpu = j+1; //Assume very simplistic cpu assignment, too annoying to deal with right now....

		}
		printf("Press any key to start applications in their respective cache partition containers\n");
		getline();
		//-----Initial static
